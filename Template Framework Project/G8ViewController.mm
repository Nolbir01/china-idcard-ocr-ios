...
-(void)recognizeImageWithTesseract:(UIImage *)img
{
    ...
    Tesseract* tesseract = [[Tesseract alloc] initWithLanguage:@"chi+eng+ita"];
    ...
    NSString *path = [[NSBundle mainBundle] pathForResource:[NSString stringWithUTF8String:"808_370231808_IMG_0885.JPG"] ofType: nil];
    IplImage* iimg = LoadImage([path fileSystemRepresentation]);
    recgonize(iimg, [NSTemporaryDirectory() fileSystemRepresentation]);
    cvReleaseImage(&iimg);
}
...
- (IBAction)recognizeSampleImage:(id)sender {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void){
        ...
        [self recognizeImageWithTesseract:[UIImage imageNamed:@"bin.jpg"]];
	});
}
